...
import VideoPlayerModal from "../../../client/components/Reviewmodal";
...
const TeacherDash = () => {
  ...
  // Handle approve topic (called from review modal)
  const handleApproveTopic = async (topicId) => {
    try {
      const res = await fetch(`/api/teacher/approve-topic`, {
        method: "POST",
        headers: {
      ...
      {loading ? (
        ...
        <>
          ...
          <VideoPlayerModal
            ...
            videoId={currentTopic?.content_id}
            onApprove={() => handleApproveTopic(currentTopic?.content_id)}
          />
        </>
      )}
    </>
